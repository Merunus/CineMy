...
const API_URL =
  "https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=16d88adebf2cbd3eb12ae86a69e43fc4";
const IMG_PATH = "https://image.tmdb.org/t/p/w1280";
const SEARCH_API =
  'https://api.themoviedb.org/3/search/movie?api_key=16d88adebf2cbd3eb12ae86a69e43fc4&query="';
...
const showMovies = (movies) => {
  ...
  movies.forEach((movie) => {
    ...
    movieEl.innerHTML = `
          ...
          <h3>${title}</h3>
            <p class = "genres-film">${getGenresForInfo(
              genres,
              genre_ids
            ).splice(0, 2)} | ${str} </p>
...
const popularitySpan = document.querySelector(
  ".modal-details-item-span-popularity"
);
const originalTitleSpan = document.querySelector(
  ".modal-details-item-span-original-title"
);
const releaseDateSpan = document.querySelector(
  ".modal-details-item-span-release-date"
);
const originalLanguageSpan = document.querySelector(
  ".modal-details-item-span-original-language"
);
...
const clearModal = () => {
  ...
  modalImage.setAttribute(
    "src",
    "https://images-we-got-pop.imgix.net/website/marketing/movie-placeholder.png?w=300&h=450&fit=clamp&auto=format,compress"
  );
  voteSpan.textContent = "";
  votesSpan.textContent = "";
  popularitySpan.textContent = "";
  originalTitleSpan.textContent = "";
  releaseDateSpan.textContent = "";
  originalLanguageSpan.textContent = "";
  overviewModal.textContent = "";
  genresModal.textContent = "";
};
const closeModal = (e) => {
    ...
    .then((videoData) => {
      if (videoData) {
        ...
        if (videoData.results.length > 0) {
          ...
          var content = `
               <h1 style = "color: white; opacity: 0.7; letter-spacing: 2px;text-transform: uppercase;">${
